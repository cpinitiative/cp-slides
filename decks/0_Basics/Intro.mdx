...
Given $n$ towers $(1 \le n \le 10^5)$, create bridges between towers such that there are $p[i][j]$ $(0 \le p[i][j] \le 2)$ different paths from tower $i$ to tower $j$ (or say that it's impossible).
